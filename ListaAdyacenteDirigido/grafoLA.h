...
class GrafoLA_Dirigido{
	...
	public:
		...
		// Adding vertex with lable 'vertex'
		void add_vertex(D vertex){
			this->tablaGrafos.put(vertex, new vector<pair<D,int>>());
		}

		// Adding edge from vertex 'a' to 'b' with a weight
		...
		// Geting index where 'vertex' is located in 'edges'
		...
		void path_dijkstra(D start, D end){
			...
			// First fill with vertices and associate inf as weight
